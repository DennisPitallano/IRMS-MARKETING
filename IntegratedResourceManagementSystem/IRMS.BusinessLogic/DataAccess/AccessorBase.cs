...
namespace IRMS.BusinessLogic.DataAccess
{
    ...
    /// Company: iServe
    /// Class Description: Data Accessor base Class
    /// </summary>
    [Log, Counter]
        ...
        where A : AccessorBase<D, A>
    {
        ...
        [Log, Counter]
        public abstract class IRMS : SqlQuery
        {
            [NoInterception]
            protected override DbManager CreateDbManager()
            {
            ...
            public virtual List<T> SelectByKeyWords<T>(string[] keyWords,  string[] columns)
            {
                ...
                for (int i = 0; i < keyWords.Length; i++)
                {
                    if (i > 0)
                        ...
                        sql.AppendFormat("LOWER(" + columns[i] + ") LIKE @kw{0} OR LOWER(" + columns[i] + ") LIKE @kw{0}", i);
        ...
        public IRMS Query
        {
            get { return TypeAccessor<IRMS>.CreateInstance(); }
        }
